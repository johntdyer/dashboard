module LabsRouting
  class Inbound
    ...
    def add
      ...
      #`printf "update add _sip._udp.#{@customer}.#{@zone_name}. 120 IN SRV 0 0 5060 #{@hostname}.\nsend\nquit" | nsupdate -y #{@key_name}:#{@tsig_key}`
    end
    ...
    def remove
      modify_host(:method=>'delete')
      #`printf "update delete _sip._udp.#{@customer}.#{@zone_name}. 120 IN SRV 0 0 5060 #{@hostname}.\nsend\nquit" | nsupdate -y #{@key_name}:#{@tsig_key}`
    end
